...
class UNetGenerator(nn.Module):
    def __init__(self):
        ...
        #self.enc9 = conv_block(512, 512)

        # Decoder layers
        #self.dec1 = deconv_block(512, 512, dropout=0.2)
        self.dec2 = deconv_block(512, 512, dropout=0.2)
        self.dec3 = deconv_block(1024, 512, dropout=0.2)
        self.dec4 = deconv_block(1024, 512)
        self.dec5 = deconv_block(1024, 256)
        self.dec6 = deconv_block(768, 128)
        self.dec7 = deconv_block(384, 64)
        self.dec8 = deconv_block(192, 32)
    ...
    def forward(self, x):
        ...
        #dec1 = self.dec1(enc9)
        #dec2 = self.dec2(torch.cat([dec1, enc8], 1))
        dec2 = self.dec2(enc8)
        dec3 = self.dec3(torch.cat([dec2, enc7], 1))
        dec4 = self.dec4(torch.cat([dec3, enc6], 1))
        dec5 = self.dec5(torch.cat([dec4, enc5], 1))
        dec6 = self.dec6(torch.cat([dec5, enc4], 1))
        dec7 = self.dec7(torch.cat([dec6, enc3], 1))
        dec8 = self.dec8(torch.cat([dec7, enc2], 1))

        output = self.final_layer(torch.cat([dec8, enc1], 1))
        return output
